	...
	$.ajax({
		...
		success : function (product, response, status) {
			...
			if (product) { 
				...
				$("#product_productPrice_edit").validatebox({
					required : true,
					validType : "number",
					missingMessage : "请输入汽车单价（万）",
					invalidMessage : "汽车单价（万）输入不对",
				});
				$("#product_productCount_edit").val(product.productCount);
				$("#product_productCount_edit").validatebox({
					required : true,
					validType : "integer",
					missingMessage : "请输入汽车库存",
					invalidMessage : "汽车库存输入不对",
				});
