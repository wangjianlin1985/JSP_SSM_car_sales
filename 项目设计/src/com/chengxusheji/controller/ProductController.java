...
public class ProductController extends BaseController {
	...
	public void add(@Validated Product product, BindingResult br,
		...
		logInfo.setLogType("汽车登记");
		logInfo.setLogContent("汽车" + product.getProductName() + "登记成功");
	...
	public String delete(@PathVariable Integer productId,HttpServletRequest request) throws UnsupportedEncodingException {
		  try {
			  productService.deleteProduct(productId);
	            request.setAttribute("message", "汽车删除成功!");
	            return "message";
	        } catch (Exception e) { 
	            e.printStackTrace();
	            request.setAttribute("error", "汽车删除失败!");
				return "error";
	...
	public void OutToExcel(@ModelAttribute("productClassObj") ProductClass productClassObj,String productName,String addTime, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        ...
        String[] headers = { "汽车id","汽车类别","汽车名称","汽车主图","汽车单价（万）","汽车库存","发布时间"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<productList.size();i++) {
        	Product product = productList.get(i); 
        	dataset.add(new String[]{product.getProductId() + "",product.getProductClassObj().getClassName(),product.getProductName(),product.getMainPhoto(),product.getProductPrice() + "",product.getProductCount() + "",product.getAddTime()});
        }
        /*
